...
(function(a, b) {
	...
	p.fn = p.prototype = {
		...
		if (!d) {
			...
			else if (e.addEventListener) e.addEventListener("DOMContentLoaded", D, !1), a.addEventListener("load", p.ready, !1);else {
	...
	p.extend({
		...
		dequeue : function(a, b) {
			...
			delete e.stop
			, d.call(a, f, e)), !c.length && e && e.empty.fire()
		},
	...
	p.event = {
		...
		var c,
			...
			$ = Z.selectors = {
				...
				filter : {
					...
					CLASS : function(a) {
						var b = M[a];
						return b || (b = M[a] = new RegExp("(^|" + r + ")" + a + "(" + r + "|$)"), N.push(a), N.length > $.cacheLength &&
							delete M[N.shift()]
							), function(a) {
		...
		var bl = Z.compile = function(a, b, c) {
			...
			return g = O[a] = bk(e), g.context = b, g.runs = g.dirruns = 0, P.push(a), P.length > $.cacheLength &&
				delete O[P.shift()]
				, g
		};
			...
			if (f)
				...
				for (j = 0, k = $.order.length; j < k; j++) {
					...
					if (s = L[p].exec(m)) {
						...
						if (h == null) continue;
						m === r && (a = a.slice(0, a.length - r.length) + m.replace(L[p], ""), a || o.apply(e, n.call(h, 0)));break
					}
				}
			}
			if (a) {
			...
			T(function(a) {
				...
				if (!g && !h && (!e || !e.test(a)))
					if (d.nodeType === 9) try {
						...
						j ? k = k.replace(c, "\\$&") : d.setAttribute("id", k);try {
	...
	bz.optgroup = bz.option, bz.tbody = bz.tfoot = bz.colgroup = bz.caption = bz.thead, bz.th = bz.td, p.support.htmlSerialize || (bz._default = [ 1, "X<div>", "</div>" ]), p.fn.extend({
		...
		clean : function(a, b, c, d) {
			...
			if (!p.support.appendChecked)
				for (s = 0; (h = t[s]) != null; s++) p.nodeName(h, "input") ? bG(h) : typeof h.getElementsByTagName != "undefined" && p.grep(h.getElementsByTagName("input"), bG);
			if (c) {
				q = function(a) {
			...
			for (; (e = a[g]) != null; g++)
				if (b || p.acceptData(e)) {
					...
					if (c) {
						...
						, j ?
							delete e[h]
							: e.removeAttribute ? e.removeAttribute(h) : e[h] = null, p.deletedIds.push(d))
					}
			}
		}
	}), function() {
					...
					c.mimeType && i.overrideMimeType && i.overrideMimeType(c.mimeType), !c.crossDomain && !e["X-Requested-With"] && (e["X-Requested-With"] = "XMLHttpRequest");try {
						...
						try {
							if (d && (e || i.readyState === 4)) {
								d = b, g && (i.onreadystatechange = p.noop, cI &&
								delete cH[g]
								);
